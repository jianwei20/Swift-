...
func distinctAuthors() -> [String] {
    ...
    for index in 0...books.count-1 {
        if author.isEmpty{
            author.append(books[index]["author"]!)
        }
        else if author[count] != books[index]["author"]{
            author.append(books[index]["author"]!)
            count += 1
        }
    }
...
func getBook(at index: Int) -> (title: String, author: String, price: Double)? {
    guard index < books.count
        else{
            return nil
    }
    let Book = books[index]
    
    guard let priceString = Book["price"], let price = Double(priceString)
        else{
            return nil
    }
